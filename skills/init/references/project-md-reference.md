...
> **Note:** This is an extended, human-readable reference for the expected `PROJECT.md` section structure, placeholder conventions, and per-section fill-in examples. The canonical **emittable** skeleton that `init` reads and writes is `../templates/PROJECT.md`; this companion file is loaded on demand for richer guidance and is not itself emitted.
